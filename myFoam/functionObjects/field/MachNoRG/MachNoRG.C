...
{
    defineTypeNameAndDebug(MachNoRG, 0);
    addToRunTimeSelectionTable(functionObject, MachNoRG, dictionary);
}
}
...
bool Foam::functionObjects::MachNoRG::calc()
{
    if
    (
    ...
    )
    {
        ...
        {
            result[i] = mag(U[i])/gasProps->c(p[i], T[i]);
        };
        ...
        forAll(result.boundaryField(), patchi)
        {
            ...
            forAll(presult, i)
            {
                presult[i] = mag(pU[i])/gasProps->c(pp[i], pT[i]);
            }
        }
...
Foam::functionObjects::MachNoRG::MachNoRG
(
    const word& name,
    const Time& runTime,
